...
public class rbrown4_UnoPlayer implements UnoPlayer {

/* HOW THIS MOFO WORKS:
 * >>The for loop checks every card. If the card is playable in any way, it gets
 * 10 points. If it's a number card, the number on the card gets added to its score.
 * A number is also added based on what percentage of my hand is in that color,
 * and the potential percentage of cards in that color in player[0]'s hand is
 * subtracted from it. The more of that color in my hand, the higher the number -
 * the more of that color in his/her hand, the lower the number.
 *
 * Card of Pain and Suffering have extra rules. First, reverses calculate the color
 * ratio for player[2] and skips calculate it for player[1] along with the color
 * ratio calculation as normal for player[0]. Then, preference is put on (1) reversing
 * away from players with very low hands or (2) reversing towards players with very
 * high hands with many CoPaS. Skips put preference on skipping players with low
 * hands. Draw_Twos also put preference on players with low hands.
 */
